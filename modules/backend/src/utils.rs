...
use crate::configs::{Config, PriceSource, TokenCreatedLog};
...
pub async fn run_price_updater(
  ...
  try_update_price(&client, &cfg, &price_source, admin.clone(), "initial")
    .await;

  loop {
    ticker.tick().await;
    try_update_price(&client, &cfg, &price_source, admin.clone(), "scheduled")
      .await;
  }
}

pub async fn try_update_price(
  client: &RpcClient,
  cfg: &Config,
  price_source: &PriceSource,
  admin: Arc<Keypair>,
...
pub async fn run_event_listener(cfg: Config) -> Result<()> {
  ...
  let (mut stream, _unsub) = client
    .logs_subscribe(
      RpcTransactionLogsFilter::Mentions(vec![cfg
        .minter_program_id
        .to_string()]),
  ...
  while let Some(value) = stream.next().await {
    if let Some(parsed) =
      parse_token_created(&value.value, cfg.minter_program_id)
    {
      info!(
...
pub fn parse_token_created(
  logs: &RpcLogsResponse,
  _program_id: Pubkey,
) -> Option<TokenCreatedLog> {
  let re = Regex::new(
...
pub fn to_fixed_6(txt: &str) -> Result<u64> {
  let val = txt.parse::<f64>()?;

  let base: u64 = 10;
  let numerator: u64 = base.checked_pow(6).ok_or(anyhow!("Math overflow"))?;
  let fixed_val = checked_mul_f64(val, numerator as f64)
...
fn checked_mul_f64(a: f64, b: f64) -> Option<f64> {
  let res = a * b;
  if res.is_finite() {
    Some(res)
  } else {
...
mod tests {
  ...
  fn parse_token_created_reads_expected_fields() {
    ...
    let parsed = parse_token_created(&logs, Pubkey::new_unique())
